...
?>
  <div class="cover">
            <div class="breadcrumb breadcrumb_product">
                <ul class="breadcrumb__list">
                    <li class="breadcrumb__item"><a href="#" class="breadcrumb__link">Главная</a>
                    </li>
                    <li class="breadcrumb__item">Каталог
                    </li>
                </ul>
            </div>
  <div class="slider">
            <div class="slider__inner">
                <div class="slider__title">Рекомендуем попробовать
                </div>
...
<div class="slider__item" id="<? echo $strMainID; ?>">
                        <div class="product">
                            <div class="product__img">
                                <div class="product__inner">
                                    <img src="<?= $arItem['PREVIEW_PICTURE']['SRC']; ?>" alt="Товар" />
                                </div>
                            </div>
		...
		<? $arFilter = array('ACTIVE' => 'Y', 'IBLOCK_ID' => $arItem['IBLOCK_ID'], 'ID'=>$arItem['IBLOCK_SECTION_ID']);
		...
		<? $arSection = $arRes; ?>
		       		        ...
		       		        <div class="product__category"><?=$arRes['NAME'];?></div>

		<? } 		?>

              
                      
                                    <div class="product__title"><a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a>
                                    </div>
                                </div>
                            
                                <div class="product__price">
                                    <div class="product__price-item" id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price">
	...
	if (!empty($minPrice))
	{
		...
		{ ?>
			<div class="product__price-item product__price-item_new">
			 <div class="product__price-title">Новая цена:</div>
             <div class="product__price-sum"><?= $minPrice['PRINT_DISCOUNT_VALUE'];?></div>
            </div>
		...
		{
			?>
			<? if (!empty($minPrice['DISCOUNT_VALUE'])) : ?>
			<div class="product__price-item product__price-item_old">
                <div class="product__price-title">Старая цена:</div>
                <div class="product__price-sum"><? echo $minPrice['PRINT_VALUE']; ?></div>
			</div><?
			endif;
		}
	}
...
?>
                                        ...
                                        <div class="product__price-value">за 100 гр.
                                        </div>
									</div>
                                </div>
                            </div>
...
?>
		...
		{
		?>
		<div class="bx_catalog_item_controls product__controls">
			<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
				<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_bt_button bx_medium btn btn_md btn_green" href="javascript:void(0)" rel="nofollow">
...
?>
				</a><a href="#" class="product__controls-click">Купить в 1 клик</a>
			</div>
<?
...
?>
				...
				<?	unset($arSectionColor);
				$rsEnum = CUserFieldEnum::GetList(array(), array("USER_FIELD_NAME"=>"UF_SECTION_COLOR", "ID" =>$arSection['UF_SECTION_COLOR'])); 
				$arEnum = $rsEnum->GetNext();
				$arSectionColor = $arEnum["VALUE"]; ?>
					                                <? if (!empty($arSectionColor)) { ?>
					                                	<span class="product__<?=$arSectionColor;?>"><?=$arSection['NAME'];?></span>
					                                <? } else { ?>
					                                	<span class="product__"><?=$arSection['NAME'];?></span>
					                                <? } ?>
							...
							{
							   if ($arProductDiscounts['SECTION_ID'] == $parentSectionId) {
								 	$arDiscount = $arProductDiscounts;
								 	$discount = intval($arDiscount['VALUE']);
							 	}
							}
							?>
							...
							<? if (!empty($discount))
							 ...
							 <span class="product__discount">Акция — <?= $discount;?>%</span>
